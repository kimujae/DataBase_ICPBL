(function(w){
    ...
    w.Hacademy.Reservation = function(area, callback){
        ...
        var seatArea = area.querySelector(".parking-seat-area");
        if(!seatArea){
            throw "좌석 영역이 지정되지 않았습니다(class='parking-seat-area')";
        }
    ...
    w.Hacademy.Reservation.prototype.addManagerEventListener = function(){
        ...
        var seatList = seatArea.querySelectorAll(".parking-seat");
    ...
    w.Hacademy.Reservation.prototype.getQueryString = function(){
        ...
        if(this.options.mode === "manager")
            checkedList = this.options.seatArea.querySelectorAll(".parking-seat:not(.empty),.parking-space");
        else if(this.options.mode === "client")
            checkedList = this.options.seatArea.querySelectorAll(".parking-seat.active");
    ...
    w.Hacademy.Reservation.prototype.setDisable = function(){
        var activeSeatList = this.options.seatArea.querySelectorAll(".parking-seat.active");
    ...
    w.Hacademy.Reservation.prototype.manualFillProcess = function(){
        ...
        for(var i=1; i <= rowsize; i++){
            for(var j=1; j <= colsize; j++){
                var findElement = cloneSeatArea.querySelector(".parking-seat[data-row='"+i+"'][data-col='"+j+"']");
                if(findElement){
    ...
    w.Hacademy.Reservation.prototype.createUnit = function(v){
        ...
        if(v === 'empty'){
            ...
            seat.classList.add("parking-space");
            ...
            seat.classList.add("parking-seat");
            seat.style.width = this.options.unitSize+"px";
            seat.style.height = this.options.unitSize+"px";
    ...
    w.Hacademy.Reservation.prototype.deleteSpace = function(){
        var checkspace = this.options.seatArea.querySelectorAll(".parking-seat.active");
    ...
    w.Hacademy.Reservation.prototype.refreshSpace = function(){
        var checkspace = this.options.seatArea.querySelectorAll(".parking-space");
    ...
    w.Hacademy.Reservation.prototype.calculateMergeSpace = function(unit){
        ...
        //top
        if(row > 0){
            var tag = this.options.seatArea.querySelector(".parking-space[data-row='"+(row-1)+"'][data-col='"+(col)+"']");
            if(tag){
        ...
        //left
        if(col > 0){
            var tag = this.options.seatArea.querySelector(".parking-space[data-row='"+(row)+"'][data-col='"+(col-1)+"']");
            if(tag){
    ...
    w.Hacademy.Reservation.prototype.changeUnit = function(unit, mode){
        ...
        if(mode === "space"){
            unit.classList.remove("active", "empty", "disabled", "parking-seat", "parking-space");
            unit.removeChild(unit.querySelector("span"));
            unit.classList.add("parking-space");
            ...
            unit.classList.remove("active", "empty", "disabled", "parking-seat", "parking-space");
            unit.textContent = "";
            unit.classList.add("parking-seat", "empty");
            unit.removeAttribute("data-direction");
            unit.dataset.state = mode;
            var newUnit = unit.cloneNode(true);
            unit.parentNode.replaceChild(newUnit, unit);
            ...
            unit.classList.add("parking-seat", "disabled");
            unit.dataset.state = mode;
            var newUnit = unit.cloneNode(true);
            ...
            if(app.options.choice === "single"){
                var list = app.options.seatArea.querySelectorAll(".parking-seat.active");
                for(var i=0; i < list.length; i++){
                    app.changeUnit(list[i], "normal");
                }
            }
            unit.classList.remove("active", "empty", "disabled", "parking-seat", "parking-space");
            unit.classList.add("parking-seat", "active");
            unit.dataset.state = mode;
        }
        else if(mode === "normal"){
            unit.classList.remove("active", "empty", "disabled", "parking-seat", "parking-space");
            unit.classList.add("parking-seat");
            unit.dataset.state = mode;
        }
    ...
    w.Hacademy.Reservation.prototype.refreshValue = function(){
        var list;
        if(this.options.mode === "manager") 
            list = this.options.seatArea.querySelectorAll(".parking-seat, .parking-space");
        else if(this.options.mode === "client"){
            list = this.options.seatArea.querySelectorAll(".parking-seat");
        }
    ...
    w.Hacademy.Reservation.prototype.resize = function(){
        ...
        var seatList = this.options.seatArea.querySelectorAll(".parking-seat");
        for(var i=0; i < seatList.length; i++){
            seatList[i].style.width = this.options.unitSize + "px";
            seatList[i].style.height = this.options.unitSize + "px";
        }            

        var spaceList = this.options.seatArea.querySelectorAll(".parking-space");
